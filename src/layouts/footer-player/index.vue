...
//- API
import { getPlayListTracks } from 'src/api/axios/spotify/playList';
...
export default defineComponent({
    ...
    setup (props) {
        ...
        const getTracks = async(playlist_id: string) => {
            await getPlayListTracks(playlist_id)
            .then((res: any) => {
                tracks.value = [...res.result.items];
        ...
        watch(
            () => playerInfo.value,
            (val) => {
                if(val) getTracks(playerInfo.value.id);
            }
        );
